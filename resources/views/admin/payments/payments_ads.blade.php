...
@section('page-header-right')
    @if(count(request()->input()))
        <a href="{{route('paymentsads')}}"> <i class="la la-arrow-circle-left"></i> {{__a('reset_filter')}}  </a>
    @endif
@endsection
            ...
            @foreach($payments as $payment)
                @if ($payment->user)
                    <tr>
                        ...
                        <td>
                            <a href="{!!route('paymentads_view', $payment->id)!!}" class="btn btn-md btn-info" data-toggle="tooltip" title="Verifikasi Pembayaran"><i class="la la-eye"></i> </a>
                            <button type="submit"> <a href="{!!route('paymentads_delete', $payment->id)!!}" class="btn btn-md btn-danger delete_confirm"><i class="la la-trash"></i> </a> </button>
                            @if ($payment->status == 0)
                                @foreach ($followUp as $fu)
                            ...
                            <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#updateDetail{{$payment->id}}"><i class="la la-edit"></i></button>
                                    <div class="modal fade" id="updateDetail{{$payment->id}}" tabindex="-1" role="dialog" aria-labelledby="updateDetail" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <form method="POST" action="{{ route('edit-paymentads', $payment->id) }}">
                                                <div class="modal-content">
                                                    <div class="modal-header">
