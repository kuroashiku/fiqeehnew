...
@section('page-js')
    <script type="text/javascript">
        ...
        function addToHome(id){
            console.log(id)
            $.post('{{ route('category_tohome') }}', {_token:'{{ csrf_token() }}', id:id, show_home:1}, function(data){
                window.location.reload(true)
            });
        }
        function removeFromHome(id){
            console.log(id)
            $.post('{{ route('category_tohome') }}', {_token:'{{ csrf_token() }}', id:id, show_home:0}, function(data){
                window.location.reload(true)
            });
        }
    </script>
