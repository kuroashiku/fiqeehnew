...
<head>
    ...
    <script>
        ...
        fbq('init', '671834363846450');
        fbq('track', 'PageView');
    </script>
    <!-- Insert Your Facebook Pixel ID below. -->
    <noscript><img height="1" width="1" style="display:none"
                   src="https://www.facebook.com/tr?id=671834363846450&amp;ev=PageView&amp;noscript=1"
        /></noscript>
        ...
        @if ($user)
            <div class="hidden lg:flex flex-grow w-full lg:items-center lg:w-auto">
                <div class="lg:flex-grow">
                    ...
                    <a href="" class="block w-full py-3 px-2 text-base font-semibold rounded-lg text-neutral-400 bg-white">
                        Afiliasi
                    </a> --}}
                    <a href="{{route('market')}}"
                       class="block mt-4 lg:inline-block lg:mt-0 mr-10 text-base font-normal text-neutral-400">
...
<div class="flex items-center text-warning-500">
    <button class="flex py-3 px-4 text-sm text-gray-900 dark:text-white" id="dropdownDefault" data-dropdown-toggle="dropdown" type="button">
        <div class="rounded-full bg-brand-50 w-10 h-10 flex items-center justify-center mr-3">
            <span class="text-base font-medium text-brand-500">{!! optional($auth_user)->get_photo !!}</span>
        </div>
        <div class="text-left">
            <p class="text-sm font-semibold text-neutral-500">
                {{ optional($auth_user)->name }}
            </p>
            @if (optional($auth_user)->user_type == 'student' && !empty(optional($auth_user)->expired_package_at) && optional($auth_user)->expired_package_at >= date('Y-m-d H:i:s'))
                <p class="text-sm font-normal text-brand-500">
                    Berakhir {{ date('d M Y', strtotime(optional($auth_user)->expired_package_at)) }}
                </p>
            @elseif (optional($auth_user)->user_type == 'admin')
                <p class="text-sm font-normal text-brand-500">
                    Admin
                </p>
            @else
...
{{-- END: Header User Dropdown --}}
        ...
        @else
            <div class="hidden lg:flex flex-grow w-full lg:items-center lg:w-auto">
                <div class="lg:flex-grow">
                    <a href="{{route('home')}}"
                       class="block mt-4 lg:inline-block lg:mt-0 mr-8 text-base font-normal text-neutral-400">
                        Home
                    </a>
                    <a href="{{route('blog')}}"
                       class="block mt-4 lg:inline-block lg:mt-0 mr-8 text-base font-normal text-neutral-400">
                ...
                <div>
                    ...
                    <a href="{{ route('search_type', 'kelas') }}"
                       class="inline-block px-9 py-2.5 rounded-lg text-base text-neutral-400 bg-white border-none">
                            <span class="material-icons-outlined">
                            search
                        </span>
                    </a>
