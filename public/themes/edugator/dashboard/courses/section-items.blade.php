        ...
        <div id="section-item-{{$item->id}}" class="edit-curriculum-item mb-2 edit-curriculum-{{$item->item_type}}">
            ...
            <div class="section-item-top border p-3 d-flex bg-white">
                ...
                @if ($item->item_type == 'lecture')
                    <a href="#" onclick="copyToClip('{{route('single_lecture_slug', [$section->course->slug, $item->slug])}}')" class="section-item-btn-tool btn px-1 py-0 section-item-edit-btn">
                        <i class="la la-link"></i>
                    </a>
                @else
                    <a href="#" onclick="copyToClip('{{route('single_'.$item->type, [$section->course->slug, $item->slug])}}')" class="section-item-btn-tool btn px-1 py-0 section-item-edit-btn">
                        <i class="la la-link"></i>
                    </a>
                @endif
