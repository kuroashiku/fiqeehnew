...
$(function () {
  ...
  $(document).on("click", ".add-item-lecture", function (e) {
    ...
    $that
      ...
      $('textarea.ajaxCkeditor').ckeditor(config);
  });
  ...
  $(document).on("click", ".is_correct_input", function (e) {
    if (
      $(this).closest("form").find("#input_option_type_radio").prop("checked")
    ) {
...
$(function(){
  $('#classcatexp').hide();
  $('#adscatexp').hide();
  $('#kategoriexp').change(function(){
      if($('#kategoriexp').val() == 1){
          $('#classcatexp').show();
          $('#adscatexp').hide();
      }else{
          $('#adscatexp').show();
...
$(document).ready(function() {
  $('#kategori').select2({
    ...
    ajax:{
      url: pageData.routes.category_select,
...
$(document).ready(function() {
  function onChangeSelect(url, id, name) {
      ...
      $.ajax({
          url: pageData.routes.category_dropdown_nl+'/'+id,
          type: 'GET',
  ...
  $('#category_product').on('change', function() {
      var id = $(this).val();
      var url = pageData.routes.category_dropdown;
      $('#product').empty().prop('disabled', false);
      onChangeSelect(url, id, 'product');
  });
...
$(document).ready(function() {
  ...
  $('.single-select-field-edit').select2({
      theme: "bootstrap-5",
      width: $(this).data('width') ? $(this).data('width') : $(this).hasClass('w-100') ? '100%' :
          'style',
      placeholder: $(this).data('placeholder'),
  ...
  $('#product_edit').prop('disabled', true);

  // on change category
  $('#category_product_edit').on('change', function() {
      var id = $(this).val();
      var url = pageData.routes.category_dropdown;
      $('#product_edit').empty().prop('disabled', false);
      onChangeSelect(url, id, 'product');
  });
