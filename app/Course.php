...
class Course extends Model
{
    ...
    protected $appends = ['thumbnail_url', 'continue_url', 'percentage'];
    ...
    {
        return $this->hasMany(CategoryCourse::class, 'course_id');
    }
    ...
    public function image_mentor()
    {
        return $this->belongsTo(Media::class, 'mentor_image_id');
    }
    ...
    public function getMentorImageUrlAttribute()
    {
        return media_image_uri($this->image_mentor)->image_sm;
    }
    ...
    public function getContinueUrlAttribute()
    {
        ...
        if ($content->item_type == 'lecture') {
            return route('single_lecture_slug', [$this->slug, $content->slug]);
        } else {
    ...
    public function status_html($badge = true)
    {
        ...
        // }
        if ($this->is_featured) {
            $html .= "<span class='badge badge-info mx-2'  data-toggle='tooltip' title='Recomendation'> <i class='la la-bookmark'></i></span>";
        }
