...
where course_user.user_id = {$this->id} and reviews.status = 1";
    ...
    public function user_enrolls()
    {
        return $this->hasMany(Enroll::class, 'user_id', 'id');
    }

    public function user_afiliasi()
    {
        return $this->hasOne(User::class, 'afiliasi_id', 'id');
    }
    ...
    {
        if ($this->options) {
            $options = (array) json_decode(json_encode($this->options), true);
