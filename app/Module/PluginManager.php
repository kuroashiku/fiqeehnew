            ...
            if ($plugin->activated) {
                if (version_compare($plugin->lms_version, $lms_version, '>')) {
                    add_action('admin_notices', function () use ($plugin, $lms_version) {
                        echo "<div class='alert alert-warning d-flex'> <p class='mb-0 mr-2' style='font-size: 35px; line-height: 1'><i class='la la-info-circle'></i></p> <p class='mb-0'> CosmoEdu is running version {$lms_version} but {$plugin->name} requires at least {$plugin->lms_version}, in order to use <strong>{$plugin->name} Plugin</strong>, please update your Teachiy LMS version.  </p> </div>";
                    });
                } else {
                    $plugin->boot();
                }
