...
class PaymentadsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->bulk_action_btn === 'update_status' && $request->status && is_array($ids) && count($ids)) {
            foreach ($ids as $id) {
                dd('1');
                $payment = PaymentAds::find($id)->first();

                PaymentAds::find($id)->update([
        ...
        $payments = PaymentAds::select(
            'payments_ads.*',
            'users.name',
            'users.email'
        )->join('users', 'payments_ads.user_id', 'users.id')->where('payments_ads.is_delete','=' ,NULL);
        if ($request->q) {
        ...
        if ($request->status != "") {
            $payments = $payments->where('payments_ads.status', $request->status);
        }
        if ($request->start_date != "") {
            $payments = $payments->where('payments_ads.created_at', '>=', $request->start_date);
        }
        if ($request->end_date != "") {
            $payments = $payments->where('payments_ads.created_at', '<=', $request->end_date);
        }
        $payments = $payments->orderBy('payments_ads.id', 'desc')->with('user')->paginate(20);
        ...
        return view('admin.payments.payments_ads', compact('title', 'payments', 'followUp'));
    }

    public function view($id)
    {
        $title = __a('payment_ads_details');
        $payment = PaymentAds::find($id);
        return view('admin.payments.ads_view', compact('title', 'payment'));
    }
    ...
    {
        return $request;
        $payment = PaymentAds::with('user')->where('id', $request->id_payment)->first();
