...
class AuthController extends Controller
{
    ...
    public function registerPost(Request $request)
    {
        ...
        if ($request->product) {
            ...
            $randStr = substr(str_shuffle(str_repeat('abcdefghijklmnopqrstuvwxyz', 5)), 0, 5);
            $file = current_disk()->putFileAs('public/uploads/payments', $request->file('file'), $randStr . '.' . $request->file('file')->extension(), 'public');
    ...
    public function sendResetToken(Request $request)
    {
        ...
        if (!$user) {
            return back()->with('error', __t('phone_not_found'));
        }
        ...
        try {
            ...
            $curl = curl_init();
                ...
                curl_setopt($curl, CURLOPT_URL, env('WOONOTIF_URL_API') . '/api/send.php?number='.$phone.'&type&message=Anda+telah+mengajukan+permintaan+untuk+mengatur+ulang+sandi+anda+di+Fiqeeh.com.+Silahkan+setel+ulang+sandi+Anda+di+'.route('reset_password_link', $user->reset_token).'&instance_id=67BFE58E02B01&access_token=648a82c5d656c');
                curl_setopt($curl, CURLOPT_URL, 'https://x3.woonotif.com/api/send.php?type=text&message=Anda+telah+mengajukan+permintaan+untuk+mengatur+ulang+sandi+anda+di+Fiqeeh.com.+Silahkan+setel+ulang+sandi+Anda+di+'.route('reset_password_link', $user->reset_token).'&number='.$phone.'&instance_id=67BFE58E02B01&access_token=648a82c5d656c');
