...
use App\Exports\DRetention;
use App\Exports\MRetention;
use App\Exports\PTRetention;
...
class DashboardController extends Controller
{
    ...
    public function reportPaymentOnly(Request $request)
    {
            ...
            $data = [];
                ...
                foreach ($payment as $vP) {
                        $data[] = [
                            ...
                            'jumlah_karyawan' => $vP->user->total_employee,
                            'product_perpanjang' => $vP->product,
                            'tanggal_perpanjang' => date('d/m/Y', strtotime($vP->last_payment)),
                            'amount_perpanjang' => $vP->amount,
                            'tanggal_daftar' => date('d/m/Y', strtotime($vP->user->created_at)),
                            'tanggal_expired' => date('d/m/Y', strtotime($vP->user->expired_package_at)),
                            'product' => $vP->user->product,
        ...
        if ($request->product != "") {
            $students = $students->where('user_payments.product', '=', $request->product);
        }
    ...
    public function reportAdsOnly(Request $request)
    {
        ...
        if ($request->category != "") {
            $students = $students->where('user_payments.category_product', '=', $request->category);
        }
        if ($request->city != "") {
            $students = $students->where('users.city', '=', $request->city);
        }
    ...
    public function reportRetensi(Request $request)
    {
        $students_leads_lea = User::select(
            'id',
            'created_at',
        ...
        $students = User::select(
            '*',
        )->where('user_type', 'student')->where('is_delete', 0)->orderBy('last_login', 'desc');
        ...
        return view('admin.reports.retensi', compact('title', 'students', 'province', 'dataLeads'));
    }

    public function exportRetensiHarian()
    {
        $data['retensi'] = User::select(
            '*',
        )->where('user_type', 'student')->where('is_delete', 0)->whereDate('last_login', date('Y-m-d'))->get();

        return Excel::download(new DRetention($data), 'Daily Retensi.xlsx');
    }
    public function exportRetensiBulanan()
    {   
        $month = Null;
        $year = Null;
        $thisYear = $year ?? date('Y');
        $thisMonth = $month ?? date('m');
        $startMonth = date("{$thisYear}-{$thisMonth}-01");
        $endMonth = date("{$thisYear}-{$thisMonth}-t");
        $data['retensi'] = User::select(
            '*',
        )->where('user_type', 'student')->where('is_delete', 0)->whereBetween('last_login',[$startMonth,$endMonth])->get();

        return Excel::download(new MRetention($data), 'Monthly Retensi.xlsx');
    }
    ...
    {   
        ...
        $data['retensi'] = User::select(
            '*',
        )->where('user_type', 'student')->where('is_delete', 0)->whereBetween('last_payment',[$startMonth,$endMonth])->get()->toArray();
        // $data['retensi'] = $students->with('user_enrolls.course.category')->get()->toArray();
        foreach ($data['retensi'] as $key => $value) {
                        $data['retensi'][$key]['payment'] = UserPayment::where('user_id', $value['id'])->where('status', 1)->orderBy('created_at', 'DESC')->first();
                        ...
                        $data['retensi'][$key]['user_enrolls'] = $dataEnroll;
                    }

        return Excel::download(new PTRetention($data), 'Perpanjangan Tempo.xlsx');
    }
    ...
    public function reportStudentAja(Request $request)
    {
        ...
        $students_leads_lea = $students_leads_lea->get();
        $dataLeads = [];
        foreach ($students_leads_lea as $key => $value) {
            $date1 = new DateTime($value['created_at']);
            $date2 = new DateTime($value['expired_package_at']);
