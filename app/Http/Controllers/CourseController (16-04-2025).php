...
class CourseController extends Controller
{
    ...
    {
        $course = Course::whereSlug($slug)->with('reviews', 'sections', 'sections.items', 'sections.items.attachments')->first();
        ...
        $course->dokumen_pendukung = 0;
        $course->video_kelas = 0;
        foreach ($course->sections as $key => $value) {
            foreach ($value->items as $key2 => $value2) {
                if (!empty($value2->video_src)) {
                    $course->video_kelas += 1;
                } else {
                    $course->dokumen_pendukung += 1;
                }
            }
        }
        // dd($course);
    ...
    {
        $lecture = Content::with('course.category')->where('slug', $lecture_id)->first();
        ...
        $content = Content::where('slug', $lecture_id)->first();
        $user = Auth::user();

        complete_content($content, $user);

        $next_course = Course::where('id', $course->next_class)->first();
        
        if ($lecture->course->category->slug == 'developer') {
            // dd($lecture->course->category->slug);
            return view(theme('marketplace'), compact('course', 'next_course', 'title', 'isEnrolled', 'lecture', 'isOpen', 'content'));
        } else {
            return view(theme('lecture'), compact('course', 'next_course', 'title', 'isEnrolled', 'lecture', 'isOpen', 'content'));
        }
    }
    ...
    public function create()
    {
        ...
        return view(theme('dashboard.courses.create_course'), compact('title', 'courses', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = [
            ...
            'price_plan'        => 'free',
            'category_id'       => $request->topic_id[0],
            'parent_category_id' => $category->category_id,
            'second_category_id' => $category->id,
            'thumbnail_id'      => $request->thumbnail_id,
            'level'             => $request->level,
            'last_updated_at'   => $now,
            'is_certificate'    => (isset($request->is_certificate)) ? $request->is_certificate : 0,
            'author'            => (isset($request->author)) ? $request->author : "Fiqeeh",
            'mentor_image_id'   => $request->mentor_image_id,
            'mentor'            => clean_html($request->mentor),
            'mentor_job'        => clean_html($request->mentor_job),
            'mentor_desc'       => clean_html($request->mentor_desc),
        ...
        foreach ($request->topic_id as $cat) {
    ...
    public function information($course_id) 
    {
        ...
        // return [$categoryCourse, $topics];
        $courses = Course::where('status', 1)->get()->toArray();
        return view(theme('dashboard.courses.information'), compact('title', 'course', 'courses', 'categories', 'topics', 'categoryCourse'));
    }
    ...
    public function informationPost(Request $request, $course_id)
    {
        ...
        CategoryCourse::where('course_id', $course_id)->delete();
        foreach ($request->topic_id as $cat) {
            CategoryCourse::insert([
                'course_id'     => $course_id,
                'category_id'   => $cat
    ...
    public function getFreeEnroll($slug)
    {
        $course = Course::with('category')->where('slug', $slug)->first();

        if (!Auth::check() && $course->category->slug != 'developer') {   
            return redirect(route('login'));
        }
        ...
        if ($course->level == 5) {
            $getCompleted = Complete::where('user_id', $user->id)->whereNotNull('completed_course_id')->orderBy('completed_course_id')->get()->toArray();
            $dataCompleted = [];
            foreach ($getCompleted as $vC) {
                $dataCompleted[] = $vC['completed_course_id'];
            }
            $getWajib = Course::where('level', 4)->whereNotIn('id', $dataCompleted)->orderBy('id')->get();
            if (count($getWajib) > 0) {
                return back()->with('warning', "Selesaikan kelas wajib terlebih dahulu");
            }
        }
        
        if ($course->category->slug == 'developer') {
            $isEnrolled = Enroll::where('course_id', $course_id)->where('user_id', 1)->first();
        } else {
            $isEnrolled = $user->isEnrolled($course_id);
        }
        ...
        if (!$course->continue_url) {
            return back(); 
        }
        return redirect($course->continue_url);
    }
    ...
    public function contentComplete($content_id)
    {
        ...
        if ($go_content) {
            ...
            $next_course = Course::where('id', $content->course->next_class)->first();
            if ($next_course) {
                return redirect(route('get_free_enroll', [$next_course->slug] ));
            } else {
                $go_content = $content;
            }

        if ($content->item_type == 'lecture') {
            return redirect(route('single_lecture_slug', [$go_content->course->slug, $go_content->slug]));
        } else {
    ...
    public function complete(Request $request, $course_id)
    {
        ...
        return view(theme('dashboard.certificate'), compact('user', 'course'));
    }
    ...
    public function myCourses(Request $request)
    {
        ...
        // return $categories;
        return view(theme('dashboard.my_courses'), compact('title', 'courses', 'categories'));
    }
