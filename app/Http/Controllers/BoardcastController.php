...
class BoardcastController extends Controller
{
    ...
    public function index2(Request $request)
    {
        ...
        $users = $users->where('user_type', 'student')->join('user_payments', 'users.id', 'user_payments.user_id' );
        ...
        if ($request->product != "") {
            $users = $users->where('users.product', '=', $request->product);
        }
        if($request->pembayaran != "") {
            $users = $users->orderBy('user_payments.user_id', 'asc')->take(1);
        ...
        $title = "Data Boardcast";
        $users = $users->where('users.is_delete', 0)->orderBy('updated_expired', 'desc')->paginate(20);
        ...
        $province = Province::get('provinsi')->whereNotNull('provinsi');
            ...
            return view('admin.boardcast.boardcast', compact('title', 'users', 'course', 'courses', 'dataLeads', 'product', 'province', 'cours','followUp'));
    } 
    ...
    public function index(Request $request)
    {
        ...
        $students5 = User::select(
            'id',
            'created_at',
            'expired_package_at'
        )->where('user_type', 'student');
        $students5 = $students5->where('expired_package_at', '<=', date('Y-m-d'));

        $students5 = $students5->get()->toArray();
        $dataLeads = [];
        foreach ($students5 as $key => $value) {
            $date1 = new DateTime($value['created_at']);
            $date2 = new DateTime($value['expired_package_at']);
            $interval = $date1->diff($date2);
            if ($interval->days > 5) {
        ...
        if ($request->status != "") {
            switch ($request->status) {
                case 'Leads':
                    $students = $students->whereIn('users.id', $dataLeads);
                    break;
                case 'Active':
                    $students = $students->where('users.expired_package_at', '>=', date('Y-m-d'));
                    break;
                case 'Expired':
                    $students = $students->whereNotIn('users.id', $dataLeads);
                    $students = $students->where('users.expired_package_at', '<=', date('Y-m-d'));
                    break;
            } 
        }
        ...
        if($request->pembayaran != "") {
            $students = $students->select('user_payments.*', DB::raw('COUNT(user_payments.user_id) as count'))
                                    ->groupBy('user_payments.user_id')
                                    ->having('count', '=', $request->pembayaran);
            // $students == $students->count('user_payments.user_id AS count')->orderBy('user_payments.created_at','asc')->having('count', '=', $request->pembayaran);
        ...
        return view('admin.boardcast.boardcast', compact('title', 'students', 'province','cours','dataLeads'));
    }
