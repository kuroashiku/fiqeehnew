    ...
    public function reportPaymentOnly(Request $request)
    {
        ...
        if ($request->filter == 'export_payment_only') {
            ...
            $data = [];
                ...
                // $userJatuhTempo = User::where('user_type', 'student')->whereBetween('created_at', [$request->start_date, $request->end_date])->get();
        ...
        // if ($request->start_date != "" && $request->end_date == "") {
        //     $students = $students->where('user_payments.created_at', '>=', $request->start_date . ' 00:00:00');
        //     $students = $students->orWhere('user_payments.expired_at', '>=', $request->start_date . ' 00:00:00');
        // } elseif ($request->start_date != "" && $request->end_date != "") {
        //     $students = $students->where(function ($query) use ($request) {
        //         $query->where('user_payments.created_at', '>=', $request->start_date . ' 00:00:00');
        //         $query->where('user_payments.created_at', '<=', $request->end_date . ' 23:59:59');
        //     });
        //     $students = $students->orWhere(function ($query) use ($request) {
        //         $query->where('user_payments.expired_at', '>=', $request->start_date . ' 00:00:00');
        //         $query->where('user_payments.expired_at', '<=', $request->end_date . ' 23:59:59');
        //     });
        // }
        ...
        //             });
        //         }
        //         $data['payments'] = $students->get()->toArray();
        //         foreach ($data['payments'] as $key => $value) {
        //             $data['payments'][$key]['user'] = User::where('id', $value['id'])->orderBy('created_at', 'DESC')->first();
        //         }
        ...
        return view('admin.reports.payment_only', compact('title', 'students'));
    }
